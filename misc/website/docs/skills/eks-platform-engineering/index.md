...
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/eks-platform-engineering/SKILL.md
format: md
---

:::info[Source]
This page is generated from [skills/eks-platform-engineering/SKILL.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/eks-platform-engineering/SKILL.md). Edit the source, not this page.
:::
...
See [references/idp-architecture.md](references/idp-architecture) for the full reference architecture and the IDP value/challenge framework.
...
1. **Provision an environment** — Backstage template → kro composes VPC + EKS Auto Mode cluster + add-ons + ArgoCD registration. Minutes, not days. → [references/infrastructure-abstraction.md](references/infrastructure-abstraction)
2. **Provision an AWS resource** — Backstage template → ACK manifest committed to Git → ArgoCD syncs → ACK provisions the AWS resource. → [references/infrastructure-abstraction.md](references/infrastructure-abstraction)
3. **Onboard an application** — Backstage template → kro `CICDPipeline` → Argo Workflows CI + ArgoCD CD. Developer contract: provide a `Dockerfile` + the platform's application manifest (a kro-rendered resource, or an OAM `Application` if you run the KubeVela abstraction); the platform does the rest. → [references/developer-portal-backstage.md](references/developer-portal-backstage), [references/gitops-delivery.md](references/gitops-delivery)
4. **Ship safely to production** — Argo Rollouts canary with gates in dev; Kargo promotes the validated artifact to prod (auto in dev, manual approval for prod). → [references/progressive-delivery.md](references/progressive-delivery)

**The developer contract** (what app teams provide vs. what the platform handles) is the design heart of every golden path — see [references/golden-paths.md](references/golden-paths).
...
Both abstractions, the CUE authoring model, the `appmod-service` example, and how to choose between them: [references/application-model-oam.md](references/application-model-oam).
...
**Kargo** orchestrates multi-stage promotion: a Warehouse watches ECR for new images; the **dev** stage auto-promotes; the **prod** stage requires manual approval and promotes the *exact same artifact* validated in dev. All promotions are Git commits (auditable, reversible). Strategies (canary/blue-green/A-B), gate config, and the Kargo Warehouse/Stage/Freight model: [references/progressive-delivery.md](references/progressive-delivery).
...
Framework and dashboards: [references/measuring-success.md](references/measuring-success).
...
Full ML/data golden paths: [references/aiml-data-platform.md](references/aiml-data-platform).
...
|-----------|------------------------------|
| [idp-architecture.md](references/idp-architecture) | IDP concept, reference architecture, hub-and-spoke, GitOps Bridge, value/challenges |
| [developer-portal-backstage.md](references/developer-portal-backstage) | Backstage portal, software templates/scaffolder, catalog, self-service flow |
| [gitops-delivery.md](references/gitops-delivery) | ArgoCD + Argo Workflows, app-of-apps, cluster registration, CI/CD wiring |
| [progressive-delivery.md](references/progressive-delivery) | Argo Rollouts strategies, quality gates, Kargo promotion |
| [infrastructure-abstraction.md](references/infrastructure-abstraction) | ACK and kro, self-service environment/resource provisioning |
| [application-model-oam.md](references/application-model-oam) | The developer-facing app model — Backstage+kro (default) vs OAM/KubeVela (alternative), components, traits, CUE |
| [golden-paths.md](references/golden-paths) | Golden-path design, guardrails, the developer contract, onboarding |
| [identity-and-tenancy.md](references/identity-and-tenancy) | Keycloak SSO, Pod Identity/IRSA, multi-tenant isolation |
| [measuring-success.md](references/measuring-success) | DORA metrics, Apache DevLake, platform dashboards |
| [genai-platform-engineering.md](references/genai-platform-engineering) | Kiro (successor to Amazon Q Developer) for templates/manifests, spec/steering workflow, prompt patterns |
| [aiml-data-platform.md](references/aiml-data-platform) | JupyterHub, Ray Serve, Kubeflow Spark Operator golden paths |
